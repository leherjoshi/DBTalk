...
export default function ChatWindow({ messages, setMessages, onAnswer }: Props) {
  ...
  const handleSubmit = async (q?: string) => {
    ...
    try {
      const response = await postQuery(questionText)
      onAnswer(response)
      // Dispatch event for history tracking
      window.dispatchEvent(new CustomEvent('query-executed', {
        detail: { question: questionText, sql: response.sql }
      }))
    } catch (err: unknown) {
        ...
        {messages.length === 0 && (
          <div style={{
            ...
            <div style={{
              ...
              <div style={{ fontSize: '11px', opacity: 0.6 }}>Click any suggestion below to get started</div>
            </div>
            <QuerySuggestions onSelect={(q) => {
              setQuestion(q)
              handleSubmit(q)
            }} />
          </div>
                ...
                {(() => {
                  ...
                  return (
                    <>
                      ...
                      />
                      {r.results.length > 0 && <ResultsTable results={r.results} sql={r.sql} />}
